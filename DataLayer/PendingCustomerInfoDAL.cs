...
namespace DataLayer
{
    public class PendingCustomerInfoDAL
    {
        /// <summary>
        ...
        public BusinessModels.Customer GetCustomer(Int32 identity)
        {
            ...
            using (var dbContext = new PendingCustomerInfoDbContext())
            {
            ...
            using (var dbContext = new PendingCustomerInfoDbContext())
            {
                ...
                _customers = dbContext.Customer
                            .Include(K => K.Location)
                             ...
                             .Include(g => g.Purpose)
                              .Include(h => h.EnquiryLevel)
                             .Where(p => p.LocationID == locID && p.AssignedTo==empID && p.IsActive == true)
                            .ToList();
            }
